(function( window, undefined ) {
			...
			if (arguments.length > 1) { // extending with a name/value pair
				...
				if (source) { // extending with an object literal 用一个对象列表来扩展
					...
					if (Base._prototyping) {
						var key,
						i = 0,
						members = ["constructor", "toString", "valueOf"];
						while (key = members[i++]) {
							//如果是重载了这些方法
	...
	if (typeof module === "object" && module && typeof module.exports === "object") { module.exports = Base; } 
	else { if (typeof define === "function" && define.amd) { define( "Base", [], function () { return Base; } ); } }
	if (typeof window === "object" && typeof window.document === "object") { window.Base = Base; }
})( window );
